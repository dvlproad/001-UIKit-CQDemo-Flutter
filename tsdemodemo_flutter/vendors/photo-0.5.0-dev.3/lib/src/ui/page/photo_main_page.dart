...
import 'package:photo/src/ui/page/photo_cell_badge.dart';
import 'package:photo/src/ui/page/photo_preview_page.dart';
import 'package:photo/src/ui/page/show_tip_util.dart';
...
class _PhotoMainPageState extends State<PhotoMainPage>
  ...
  String get currentGalleryName {
    AssetPathEntity assetPathEntity = assetProvider.current;
  ...
  bool _hasPrefixWidget; // if exsit prefix widget
  ...
  void initState() {
    ...
    _hasPrefixWidget = widget.prefixWidget != null;
  }
  ...
  Widget _buildBody() {
    ...
    var count = assetProvider.count + (noMore ? 0 : 1);
    if (_hasPrefixWidget == true) {
      count++;
    }
  ...
  Widget _buildItem(BuildContext context, int index) {
    if (_hasPrefixWidget == true && index == 0) {
    ...
    final noMore = assetProvider.noMore;
    if (!noMore && assetIndex == assetProvider.count) {
      _loadMore();
      return _buildLoading();
    }

    var data = list[assetIndex];
    return RepaintBoundary(
      child: GestureDetector(
        onTap: () => _onItemClick(data, assetIndex),
        child: Stack(
          children: <Widget>[
            ImageItem(
              entity: data,
  ...
  Widget _buildSelected(AssetEntity entity) {
    ...
    if (containsEntity(entity)) {
      text = (indexOfSelected(entity) + 1).toString();
    }
  ...
  void changeCheck(bool value, AssetEntity entity) {
    if (value) {
      SelectedProviderAddErrorCode errorCode = addSelectEntity(entity);
      if (errorCode == SelectedProviderAddErrorCode.upperLimit) {
        ShowTipUtil.showTip(
          context: scaffoldKey.currentContext,
          i18nProvider: i18nProvider,
          options: options,
