...
abstract class TXVodPlayEvent {
  ...
  static const PLAY_EVT_RTMP_STREAM_BEGIN =
      2002; // 已经连接服务器，开始拉流（仅播放 RTMP 地址时会抛送）
  ...
  static const PLAY_EVT_PLAY_LOADING =
      2007; // 视频播放进入缓冲状态，缓冲结束之后会有 PLAY_BEGIN 事件
  static const PLAY_EVT_START_VIDEO_DECODER = 2008; // 视频解码器开始启动（2.0 版本以后新增)
  static const PLAY_EVT_CHANGE_RESOLUTION =
      2009; // 视频分辨率发生变化（分辨率在 EVT_PARAM 参数中）
  static const PLAY_EVT_GET_PLAYINFO_SUCC =
      2010; // 如果您在直播中收到此消息，说明错用成了 TXVodPlayer
  static const PLAY_EVT_CHANGE_ROTATION =
      2011; // 如果您在直播中收到此消息，说明错用成了 TXVodPlayer
  static const PLAY_EVT_GET_MESSAGE =
      2012; // 获取夹在视频流中的自定义 SEI 消息，消息的发送需使用 TXLivePusher
  ...
  static const PLAY_WARNING_RECONNECT =
      2103; // 网络断连，已启动自动重连（重连超过三次就直接抛送 PLAY_ERR_NET_DISCONNECT）
  static const PLAY_WARNING_RECV_DATA_LAG =
  ...
  static const PLAY_WARNING_SEVER_CONN_FAIL =
      3002; // RTMP 服务器连接失败（仅播放 RTMP 地址时会抛送）
  ...
  static const EVT_GET_MSG =
      "EVT_GET_MSG"; // 消息内容，收到PLAY_EVT_GET_MESSAGE事件时，通过该字段获取消息内容
  ...
  static const EVENT_PIP_MODE_UI_STATE_CHANGED =
      4; // pip UI状态发生变动，only support android > 31
  ...
  static const ERROR_IOS_PIP_DEVICE_NOT_SUPPORT =
      -104; // pip 错误，设备或系统版本不支持（iPad iOS9+ 才支持PIP）
  static const ERROR_IOS_PIP_PLAYER_NOT_SUPPORT =
      -105; // pip 错误，播放器不支持 only support iOS
  static const ERROR_IOS_PIP_VIDEO_NOT_SUPPORT =
      -106; // pip 错误，视频不支持 only support iOS
  static const ERROR_IOS_PIP_IS_NOT_POSSIBLE =
      -107; // pip 错误，PIP控制器不可用 only support iOS
  static const ERROR_IOS_PIP_FROM_SYSTEM =
      -108; // pip 错误，PIP控制器报错 only support iOS
  static const ERROR_IOS_PIP_PLAYER_NOT_EXIST =
      -109; // pip 错误，播放器对象不存在 only support iOS
  static const ERROR_IOS_PIP_IS_RUNNING =
      -110; // pip 错误，PIP功能已经运行 only support iOS
  static const ERROR_IOS_PIP_NOT_RUNNING =
      -111; // pip 错误，PIP功能没有启动 only support iOS
...
abstract class TXVodNetEvent {
  ...
  static const NET_STATUS_VIDEO_GOP =
      "VIDEO_GOP"; // 当前视频GOP,也就是每两个关键帧(I帧)间隔时长，单位s
  static const NET_STATUS_VIDEO_BITRATE =
      "VIDEO_BITRATE"; // 推流：视频数据发送比特率；拉流：视频数据接收比特率 单位：kbps
  static const NET_STATUS_AUDIO_BITRATE =
      "AUDIO_BITRATE"; // 推流：音频数据发送比特率；拉流：音频数据接收比特率 单位：kbps
  static const NET_STATUS_NET_SPEED =
      "NET_SPEED"; // 推流：音视频数据发送总比特率；拉流：音视频数据接收总比特率 单位：kbps
  static const NET_STATUS_AUDIO_CACHE =
      "AUDIO_CACHE"; // 推流：发送端缓存未发送的音频帧数；拉流：接收端已接收但未播放的音频帧总时长
  static const NET_STATUS_VIDEO_CACHE =
      "VIDEO_CACHE"; // 推流：发送端缓存未发送的视频帧数；拉流：接收端已接收但未渲染的视频帧总时长
  static const NET_STATUS_AUDIO_DROP =
      "AUDIO_DROP"; // 推流：发送端音频丢帧数(未用:上行无音频丢帧逻辑)   拉流：接收端音频丢帧数（未用：播放端有音频加速，不丢帧）
  static const NET_STATUS_VIDEO_DROP =
      "VIDEO_DROP"; // 推流：发送端视频丢帧数(有用:实时推流有丢帧逻辑)   拉流：接收端视频丢帧数（未用：播放端有视频加速，不丢帧）
  static const NET_STATUS_V_SUM_CACHE_SIZE =
      "V_SUM_CACHE_SIZE"; // 拉流专用：接收端已接收但未渲染的视频帧数（包括JitterBuffer和解码器两部分缓存）
  static const NET_STATUS_V_DEC_CACHE_SIZE =
      "V_DEC_CACHE_SIZE"; // 拉流专用：接收端解码器里缓存的视频帧数
  ...
  static const NET_STATUS_AUDIO_BLOCK_TIME =
      "AUDIO_BLOCK_TIME"; // 拉流专用：音频卡顿时长，单位ms
  static const NET_STATUS_AUDIO_INFO =
      "AUDIO_PLAY_INFO"; // 当前流的音频信息，包括采样率信息和声道数信息
  ...
  static const NET_STATUS_QUALITY_LEVEL =
...
class TXLogLevel {
  ...
  static const LOG_LEVEL_DEBUG =
      1; // 输出 DEBUG,INFO,WARNING,ERROR 和 FATAL 级别的log
...
class TXPlayInfoParams {
  ...
  final String?
      psign; // encent cloud video encryption signature, required for encrypted video

  const TXPlayInfoParams(
      {required this.appId, required this.fileId, this.psign});
...
typedef FTXDownlodOnStateChangeListener = void Function(
    int event, TXVodDownloadMedialnfo info);
typedef FTXDownlodOnErrorListener = void Function(
    int errorCode, String errorMsg, TXVodDownloadMedialnfo info);
