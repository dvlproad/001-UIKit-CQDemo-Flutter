// qian: 原本无法实现当只显示 月日时分 的时候，可以指定最大最小值，所以fork后修改
...
import 'picker_time_util.dart';

class ForkDatePickerRoute<T> extends PopupRoute<T> {
  final int? minuteMultiple;
  ForkDatePickerRoute({
    this.minuteMultiple,
    this.mode,
  ...
  final void Function(PDuration res)? onChanged;
  final void Function(PDuration res)? onConfirm;
...
class _PickerContentView extends StatefulWidget {
  ...
  late final ForkDatePickerRoute route;
  ...
  State<StatefulWidget> createState() =>
      ...
      _PickerState(mode, initData, maxDate, minDate, pickerStyle);
}
...
class _PickerState extends State<_PickerContentView> {
  ...
  void initState() {
    super.initState();
    _minuteMultiple = widget.minuteMultiple;
  ...
  _init() {
    ...
    if (_dateItemModel.year) {
      ...
      _selectData.year = minDate.year!; // qian fork:
    }
    ...
    if (_dateItemModel.hour) {
      ...
      if (intNotEmpty(minDate.hour) &&
          PickerTimeUtils.isTimeSamePre(_selectData, minDate, DateMode.YMD)) {
        begin = minDate.hour!;
      }
      if (intNotEmpty(maxDate.hour) &&
          PickerTimeUtils.isTimeSamePre(_selectData, maxDate, DateMode.YMD)) {
        end = maxDate.hour!;
      }
    ...
    if (_dateItemModel.minute) {
      ...
      if (intNotEmpty(minDate.minute) || intNotEmpty(maxDate.minute)) {
        if (_dateItemModel.hour) {
          // 如果有上级 还有时，要根据时再判断
          if (PickerTimeUtils.isTimeSamePre(
              _selectData, minDate, DateMode.YMDH)) {
            begin = minDate.minute!;
          }
          if (PickerTimeUtils.isTimeSamePre(
              _selectData, maxDate, DateMode.YMDH)) {
            end = maxDate.minute!;
          }
        } else {
          // 上级没有时间限制 直接取
      ...
      _dateTimeData.minute = PickerTimeUtils.calcMinAndSecond(
        begin: begin,
        end: end,
        multiple: _minuteMultiple,
      );
      if (_dateTimeData.minute.isEmpty) {
        debugPrint(
            'error============ 确实分钟数据，会导致滚轮定位错误，请检查，有可能是你的分钟只能是00 15 30 45，但是你现在已经是46分了，所以遇到这种情况请在传入的选中时间上调整为下一个时段的，如17:46改为18:00');
      }
  ...
  void _setDay() {
    ...
    if (_dateItemModel.hour) {
      ...
      int endHour = 23;
      // 限制区域
      if (PickerTimeUtils.isTimeSamePre(_selectData, minDate, DateMode.YMD)) {
        beginHour = minDate.hour!;
      }
      if (PickerTimeUtils.isTimeSamePre(_selectData, maxDate, DateMode.YMD)) {
        endHour = maxDate.hour!;
      }
      resultHour = TimeUtils.calcHour(begin: beginHour, end: endHour);

      if (!listEquals(_dateTimeData.hour, resultHour)) {
        //可能 选中的年 月份 由于设置了新数据后没有了
        // 小于不用考虑 会进else
        if (_selectData.hour! > resultHour.last) {
          jumpToIndexHour = resultHour.length - 1;
        } else {
          jumpToIndexHour = resultHour.indexOf(_selectData.hour);
        }
        jumpToIndexHour = jumpToIndexHour < 0 ? 0 : jumpToIndexHour;
        _selectData.hour = resultHour[jumpToIndexHour];
        updateHour = true;
      }
    }
    if (updateHour) {
      setState(() {
        _dateTimeData.hour = resultHour;
        scrollCtrl[DateType.Hour]?.jumpToItem(jumpToIndexHour);
  ...
  void _setHour() {
    ...
    if (_dateItemModel.minute) {
      ...
      var resultMinute = PickerTimeUtils.calcMinAndSecond(
        begin: beginMinute,
        end: endMinute,
        multiple: _minuteMultiple,
      );
      if (resultMinute.isEmpty) {
        debugPrint(
            'error============ 确实分钟数据，会导致滚轮定位错误，请检查，有可能是你的分钟只能是00 15 30 45，但是你现在已经是46分了，所以遇到这种情况请在传入的选中时间上调整为下一个时段的，如17:46改为18:00');
      }
