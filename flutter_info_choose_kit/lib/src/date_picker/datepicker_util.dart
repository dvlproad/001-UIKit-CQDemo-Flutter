...
class DatePickerUtil {
  ...
  static void chooseBirthday(
    ...
    if (selectedyyyyMMddDateString != null &&
        selectedyyyyMMddDateString.isNotEmpty) {
      DateTime yyyyMMddDate = DateTime.parse(selectedyyyyMMddDateString);
      selectedyyyyMMddDate = yyyyMMddDate;
    }
    */
    DateTime currentDate = DateTime.now();
    DateTime minDate = DateTime(currentDate.year - maxAge, 1, 1);
    DateTime maxDate =
        DateTime(currentDate.year - minAge, currentDate.month, currentDate.day);

    if (selectedyyyyMMddDateString == null ||
        selectedyyyyMMddDateString.isEmpty) {
      selectedyyyyMMddDateString = DateFormat('yyyy-MM-dd').format(maxDate);
    }
    DatePickerUtil.chooseyyyyMMddFuture(
      context,
      title: title,
      selectedyyyyMMddDateString: selectedyyyyMMddDateString,
      minDateString: DateFormat('yyyy-MM-dd').format(minDate),
      maxDateString: DateFormat('yyyy-MM-dd').format(maxDate),
  ...
  static void chooseyyyyMMddFuture(
    ...
    if (minDateTime != null && maxDateString != null) {
      ...
      if (compareResult == 0) {
        ...
        debugPrint("Error：日期错误，最大日期不能小于最小日期，请检查");
        return;
      }
    }
    ...
    Pickers.showDatePicker(
      ...
      */
      mode: dateMode,
      selectDate: PDuration.parse(selectedyyyyMMddDate),
      ...
      onConfirm: (p) {
        ...
        // debugPrint("yyyyMMddDateString=$yyyyMMddDateString");
...
class DateFormatterUtil {
  ...
  static DateTime? fromDateString(String? selectedyyyyMMddDateString) {
    ...
    if (selectedyyyyMMddDateString == null ||
        selectedyyyyMMddDateString.isEmpty ||
        !selectedyyyyMMddDateString.contains("-")) {
